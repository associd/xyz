...
function get_file($suffix, $path) {
    ...
    foreach(scandir($path) as $val) {
        if($val == '.' || $val == '..') {
            ...
            foreach($suffix as $v) {
                if(strpos($val, $v)) {
                    array_push($arr,$path. '/' .$val);
                }
            }
        }else {
            $arr = array_merge(get_file($suffix, $path.'/'.$val), $arr);
        }
    }
    return $arr;
}
...
function read_mp3($file, $img_path) {
    $frame_name = [
        'Name' => 'TIT2',
        'Composer' => 'TPE1',
        'Album' => 'TALB',
...
//        fseek($fp, 4);
//        $deputy = fread($fp, 1);
//        $result['version'] = ord($version);
//        $result['deputy'] = ord($deputy);

        //计算整个标签大小 单位字节
        fseek($fp, 6);
        $size = str_split(fread($fp, 4));
        $sum  = (ord($size[0]) * 0x200000)
            + (ord($size[1]) * 0x4000)
            + (ord($size[2]) * 0x80)
            + ord($size[3]);
        $label_size = $sum;
        ...
        $name_size = 4;
        $size_size = 4;
        while($offset < $label_size) {
            //帧名字
            fseek($fp, $offset);
            $name = fread($fp, $name_size);
            $offset += $name_size;
            $num = 0;
            foreach(str_split($name) as $val) {
                if(ord($val) == 0) {
                    $num ++;
                }
            }
            ...
            //帧大小
            fseek($fp, $offset);
            $size = str_split(fread($fp, $size_size));
            $sum  = (ord($size[0]) * 0x1000000)
                + (ord($size[1]) * 0x10000)
                + (ord($size[2]) * 0x100)
                + ord($size[3]);
            $frame_size = $sum;
...
//                        $type = fread($fp, 4);
                        ...
                        fseek($fp, $offset + 7 + $start);
                        $content = fread($fp, $frame_size);

                        //创建 img 文件
                        $file_name = date('Y_m_d').'_'.uniqid();
                        $url = "$img_path/$file_name.$type";
                        $img = fopen("$url", "wb");
                        fwrite($img, $content);
                        fclose($img);
                        $result["$key"] = $url;
                    }else {
                        $content = fread($fp, $frame_size - 1);
                        $content = iconv($encode[$index], 'utf-8', $content);
                        $result["$key"] = $content;
                    }
                }
            }
